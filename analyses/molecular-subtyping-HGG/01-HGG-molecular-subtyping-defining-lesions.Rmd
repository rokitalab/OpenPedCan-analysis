...
```{r}
# Get LGG samples on the basis of pathology diagnosis
...
snv_dgd_maf <- data.table::fread(
  file.path(root_dir, "data" , "snv-dgd.maf.tsv.gz"),
                                   select = keep_cols,
                                   data.table = FALSE) %>%
  dplyr::filter(!Tumor_Sample_Barcode %in% lgat_specimens) 
...
snv_lesions_df <- snv_consensus_hotspot_maf  %>%
  ...
  dplyr::mutate(
    "H3-3A.K28M" = dplyr::case_when(Hugo_Symbol == "H3-3A" &
                                    HGVSp_Short == "p.K28M" ~ "Yes",
                                  TRUE ~ "No"),
    H3C2.K28M = dplyr::case_when(
      Hugo_Symbol == "H3C2" & HGVSp_Short == "p.K28M" ~ "Yes",
      TRUE ~ "No"),
    H3C3.K28M = dplyr::case_when(
      Hugo_Symbol == "H3C3" & HGVSp_Short == "p.K28M" ~ "Yes",
      TRUE ~ "No"),
    H3C14.K28M = dplyr::case_when(
      Hugo_Symbol == "H3C14" & HGVSp_Short == "p.K28M" ~ "Yes",
      TRUE ~ "No"),
    "H3-3A.G35R" = dplyr::case_when(Hugo_Symbol == "H3-3A" &
                                    HGVSp_Short == "p.G35R" ~ "Yes",
                                  TRUE ~ "No"),
    "H3-3A.G35V" = dplyr::case_when(Hugo_Symbol == "H3-3A" &
