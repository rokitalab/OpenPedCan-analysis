...
methyl_df <- hgg_meta %>% 
  filter(experimental_strategy == "Methylation", 
         (grepl("GBM_|pedHGG_|HGG_", dkfz_v12_methylation_subclass) |
             dkfz_v12_methylation_subclass %in% c("DHG_G34", "DMG_K27", "DMG_EGFR", "A_IDH_HG", "IHG", "HGAP", "A_IDH_HG", "PXA")), 
...
pxa_df_biospecimen <- all_data_df %>% 
  # need free text
    left_join(unique(hgg_meta[,c("pathology_free_text_diagnosis", "match_id")])) %>%
  filter(molecular_subtype_methyl == "PXA" | grepl("pleomorphic xanthoastrocytoma|pxa", pathology_free_text_diagnosis) & BRAF_V600E %in% c("p.V600E", "p.V640E") & 
