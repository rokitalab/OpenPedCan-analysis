...
author: "Chante Bethell, Jaclyn Taroni for ALSF CCDL and Zhuangzhuang Geng for D3b"
date: "2020"
output:
  html_notebook:
    toc: TRUE
...
dna_df <- mutation_df %>%
  ...
  left_join(histologies_df[, c("Kids_First_Participant_ID", "sample_id", "Kids_First_Biospecimen_ID", "composition")])
```
...
```{r}
starfusion_df <- read_tsv(file.path(data_dir, "fusion-starfusion.tsv.gz"))
```
...
```{r}
arriba_df <- read_tsv(file.path(data_dir, "fusion-arriba.tsv.gz"))
```
