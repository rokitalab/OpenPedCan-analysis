...
```{r}
all_data_df <- bind_rows(dna_df, rna_df)
```
...
methyl_df <- hgg_meta %>% 
  ...
  dplyr::rename(Kids_First_Biospecimen_ID = Kids_First_Biospecimen_ID) %>%
  distinct()

# add remaining methyl without the subtypes of interest 
methyl_not_subtyped <- hgg_meta %>%
  filter(experimental_strategy == "Methylation",
         !Kids_First_Biospecimen_ID %in% methyl_df$Kids_First_Biospecimen_ID) %>%
  dplyr::rename(Kids_First_Biospecimen_ID = Kids_First_Biospecimen_ID) %>%
  select(Kids_First_Biospecimen_ID, Kids_First_Participant_ID, sample_id, 
                composition, id) %>%
...
ihg_df_biospecimen <- all_data_df %>% 
  ...
  unique() %>%
  mutate(molecular_subtype = case_when((!is.na(tmp_tag) & Kids_First_Biospecimen_ID %in% NTRK ~ "IHG, NTRK-altered"),
                                       (!is.na(tmp_tag) & Kids_First_Biospecimen_ID %in% ALK ~ "IHG, ALK-altered"),
                                       (!is.na(tmp_tag) & Kids_First_Biospecimen_ID %in% ROS1 ~ "IHG, ROS1-altered"),
                                       (!is.na(tmp_tag) & Kids_First_Biospecimen_ID %in% MET ~ "IHG, MET-altered"),
                                       (!is.na(tmp_tag) & !Kids_First_Biospecimen_ID %in% c(NTRK, ALK, MET, ROS1) & molecular_subtype_methyl == "IHG") ~ "IHG, To be classified")) %>%
...
wildtype_methyl <- all_data_df %>%
  filter(!Kids_First_Biospecimen_ID %in% wildtype_df$Kids_First_Biospecimen_ID,
