...
## collect all dna and rna ids to add to dfs later, in case of no fusion or mutation/cnv calls
```{r sample lists}
rna_bsids <- hgg_meta %>%
...
not_subtyped <- setdiff(all_data_df$id, molecular_subtype_table$id)
...
wildtype_df <- all_data_df %>%
  ...
  filter(is.na(`H3F3A_H3-3A.K28M`) | `H3F3A_H3-3A.K28M` == "No",
         is.na(HIST1H3B_H3C2.K28M) | HIST1H3B_H3C2.K28M == "No",
         is.na(HIST1H3C_H3C3.K28M) | HIST1H3C_H3C3.K28M == "No",
         is.na(HIST2H3C_H3C14.K28M) | HIST2H3C_H3C14.K28M == "No",
         is.na(`H3F3A_H3-3A.G35R`) | `H3F3A_H3-3A.G35R` == "No",
         is.na(`H3F3A_H3-3A.G35V`) | `H3F3A_H3-3A.G35V` == "No",
         is.na(IDH1_mutation) | IDH1_mutation == "No R132 or R172",
...
```{r add remaining bs_ids}
missing_methyl <- hgg_meta %>%
  filter(Kids_First_Biospecimen_ID %in% setdiff(methyl_bsids$Kids_First_Biospecimen_ID_Methyl, methyl_df$Kids_First_Biospecimen_ID_Methyl)) %>%
  select(Kids_First_Biospecimen_ID_Methyl = Kids_First_Biospecimen_ID, id)

molecular_subtype_table_final <- molecular_subtype_table %>%
  bind_rows(wildtype_df)

# subtype mapping 
subtype_map <- molecular_subtype_table_final %>%
  select(id, molecular_subtype, molecular_subtype_methyl) %>%
  unique()
as.data.frame(table(subtype_map$molecular_subtype)) 

# add missing methyl
missing_methyl_subtyped <- missing_methyl %>%
  left_join(subtype_map) %>%
  # to be classified if NA
  mutate(molecular_subtype = case_when(is.na(molecular_subtype) ~ "HGG, To be classified",
                                    TRUE ~ molecular_subtype))

molecular_subtype_table_final <- molecular_subtype_table_final %>%
  bind_rows(missing_methyl_subtyped) %>%
  arrange(id)

# subset only columns needed
molecular_subtype_table_final_subset <- molecular_subtype_table_final %>%
  select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID_DNA, Kids_First_Biospecimen_ID_RNA, Kids_First_Biospecimen_ID_Methyl, sample_id, composition, id, molecular_subtype, molecular_subtype_methyl) %>%
  unique()
```

## Finally, do we have all bs_ids accounted for?

```{r check bs ids}
ids <- c(molecular_subtype_table_final_subset$Kids_First_Biospecimen_ID_DNA, molecular_subtype_table_final_subset$Kids_First_Biospecimen_ID_RNA, 
         molecular_subtype_table_final_subset$Kids_First_Biospecimen_ID_Methyl) %>% unique()

all_needed_ids <- hgg_meta$Kids_First_Biospecimen_ID

missing_dna <- setdiff(dna_bsids$Kids_First_Biospecimen_ID_DNA, molecular_subtype_table_final_subset$Kids_First_Biospecimen_ID_DNA)
missing_dna

missing_rna <- setdiff(rna_bsids$Kids_First_Biospecimen_ID_RNA, molecular_subtype_table_final_subset$Kids_First_Biospecimen_ID_RNA)
missing_rna

missing_methyl <- setdiff(methyl_bsids$Kids_First_Biospecimen_ID_Methyl, molecular_subtype_table_final_subset$Kids_First_Biospecimen_ID_Methyl)
missing_methyl

dna_missing_df <- hgg_meta %>%
  filter(Kids_First_Biospecimen_ID %in% missing_dna)

rna_missing_df <- hgg_meta %>%
  filter(Kids_First_Biospecimen_ID %in% missing_rna)
...
```{r}
write_tsv(molecular_subtype_table_final, lesion_subtype_file)
write_tsv(molecular_subtype_table_final_subset, subtype_file)
```
