...
methyl_df <- hgg_meta %>% 
  filter(experimental_strategy == "Methylation", 
         (grepl("GBM_|pedHGG_|HGG_", dkfz_v12_methylation_subclass) |
             dkfz_v12_methylation_subclass %in% c("DHG_G34", "DMG_K27", "DMG_EGFR", "A_IDH_HG", "IHG", "HGAP", "A_IDH_LG", "A_IDH_HG", "DGONC", "PXA"))
         ) %>% 
  # add all samples but only keep subtypes if score >=0.8
  mutate(molecular_subtype_methyl = case_when(
    dkfz_v12_methylation_subclass_score >= 0.8 & grepl("GBM_|pedHGG_|HGG_", dkfz_v12_methylation_subclass) ~ "HGG, H3 wildtype",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "DHG_G34"  ~ "DHG, H3 G35",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass %in% c("DMG_K27", "DMG_EGFR")  ~ "DMG, H3 K28",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "A_IDH_HG"  ~ "HGG, IDH",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "O_IDH" ~ "Oligo, IDH",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "IHG"  ~ "IHG",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "HGAP"  ~ "HGG, MAPK",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "A_IDH_LG"  ~ "LGG, IDH",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "DGONC"  ~ "GNT",
    dkfz_v12_methylation_subclass_score >= 0.8 & dkfz_v12_methylation_subclass == "PXA"  ~ "PXA",
    TRUE ~ NA_character_)) %>% 
...
all_subtypes <- subtype_map %>%
  ...
  select(Kids_First_Biospecimen_ID, id, molecular_subtype)
