...
clinical<-read_tsv(file.path(root_dir, params$histology))
# get grouping column id
group<-params$group
# subset to group
clinical<-clinical[,c("Kids_First_Biospecimen_ID","Kids_First_Participant_ID",group)]
} else {
clinical<-read_tsv(file.path(root_dir, params$base_histology))
# get grouping column id
...
fusion_calls <- fusion_calls %>%
  ...
  # ARRIBA and STARFUSION don't use the same names for FusionType, so if 
  # different keeping the ARRIBA name because there are more options which are
  # more descriptive
  # group_by(Sample, FusionName, LeftBreakpoint, RightBreakpoint) %>%
  # mutate(n_types = n_distinct(Fusion_Type)) %>%
  # ungroup() %>% 
  # dplyr::filter(ifelse(n_types != 1, Caller == 'ARRIBA', Caller == Caller)) %>%
  # dplyr::select(-Caller, -n_types) %>%
  ...
  # group_by(Sample, FusionName, LeftBreakpoint, RightBreakpoint) %>%
  # dplyr::mutate(DomainRetainedGene1A = toString(unique(DomainRetainedGene1A)), 
  #                  DomainRetainedGene1B = toString(unique(DomainRetainedGene1B))) %>%
  # ungroup() %>%
  # # collapse any remaining duplicate fusion types
  # group_by(Sample, FusionName, LeftBreakpoint, RightBreakpoint) %>%
  # dplyr::mutate(Fusion_Type = toString(unique(Fusion_Type))) %>%
  # ungroup() %>%
  unique()
