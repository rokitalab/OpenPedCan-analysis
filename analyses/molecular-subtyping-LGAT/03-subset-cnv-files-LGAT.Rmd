...
manta_gr <- manta_lgat %>%
  # format
  dplyr::rename(
    chr = SV_chrom, start = SV_start, end = SV_end,
    ALT = ALT
  ) %>%
  filter(Gene_name == "FGFR1" ,
...
fgfr1_tx_id <- ensembldb::select(txdb, 
  ...
  dplyr::filter(TXNAME == "ENST00000397091.9") %>%
  pull(TXID)
...
LGAT_cnv_subset <- as.data.frame(lgat_wgs_biospecimen_ids) %>%
  ...
  ## add match_id 
  left_join(lgat_specimens_df %>% select(Kids_First_Biospecimen_ID, match_id), 
            by = c("lgat_wgs_biospecimen_ids" = "Kids_First_Biospecimen_ID"))
