...
### SNV for subtyping 
 - We will use the `snv-consensus-plus-hotspots.maf.tsv.gz` which contained consensus calling plus hotspot rescued for SNV calling - detialed in [here](https://github.com/AlexsLemonade/OpenPBTA-analysis/tree/master/analyses/snv-callers#snv-caller-comparison-analysis)). Also include the DGD MAF, `snv-dgd.maf.tsv.gz` for OpenPedCan.
...
lgat_methyl_subtyped <- clinical %>% 
  ...
  dplyr::mutate(molecular_subtype_methyl = 
                  case_when(grepl("LGG_MYB", dkfz_v12_methylation_subclass) ~ "LGG, MYB/MYBL1",
                            ...
                            dkfz_v12_methylation_subclass %in% c("PA_INF_FGFR", "PA_INF")  ~ "LGG, FGFR",
...
lgat_methyl_not_subtyped <- clinical %>% 
  ...
  dplyr::mutate(molecular_subtype_methyl = case_when(
    is.na(molecular_subtype_methyl) ~ "LGG, To be classified",
