...
# clinical file
clinical_file <- read_tsv(file.path(root_dir, "data", "histologies-base.tsv"), 
                          guess_max = 100000)
...
methly_subtyped <- read_tsv(file.path(subset_dir,"LGAT_methyl_subtypes.tsv"))
...
### Compile all DNA and RNA subtypes
...
all_subtype <- methly_subtyped %>%
  dplyr::full_join(all_subtype, by = c("Kids_First_Participant_ID", "sample_id")) %>%
  dplyr::mutate(molecular_subtype = case_when(
...
### Inlcude LGG Methylation to be classified 

```{r}

all_subtype <- clinical_file %>% 
  dplyr::filter(str_detect(str_to_lower(pathology_diagnosis), 
                           paste(c(path_dx_list$include_path_dx, "Dysplasia/Gliosis"), collapse = "|")),
                !Kids_First_Biospecimen_ID %in% unique(all_subtype$Kids_First_Biospecimen_ID_Methyl),
                experimental_strategy == "Methylation",
                sample_type == "Tumor", 
                composition == "Solid Tissue",
                cohort != "TARGET") %>% 
  dplyr::select(Kids_First_Biospecimen_ID, Kids_First_Participant_ID,	sample_id, molecular_subtype) %>%
  dplyr::rename("Kids_First_Biospecimen_ID_Methyl" = "Kids_First_Biospecimen_ID") %>%
  dplyr::mutate(molecular_subtype = "LGG, To be classified", molecular_subtype_methyl = "LGG, To be classified") %>% 
  dplyr::full_join(all_subtype, by = c("Kids_First_Biospecimen_ID_Methyl", "Kids_First_Participant_ID",	"sample_id",
...
### Update a patient classified with multiple LGG subtypes

```{r}
all_subtype <- all_subtype %>% 
  dplyr::mutate(molecular_subtype = 
                  case_when((Kids_First_Participant_ID == "PT_2JGWN6EA" & 
                               sample_id == "7316-8051") ~ "LGG, NF1-somatic, FGFR",
                            TRUE ~ molecular_subtype)) %>% 
  dplyr::distinct() %>% 
