...
clinical_file <- read_tsv(file.path(root_dir, "data", "histologies-base.tsv"), guess_max = 100000) %>%
    dplyr::mutate(id = paste(sample_id, composition, sep = "_"))
...
all_lgat_matched <- full_join(lgat_dna, lgat_rna,
                         by = c("Kids_First_Participant_ID",
                                ...
                                "pathology_diagnosis"),
                         suffix = c("_DNA", "_RNA")) %>%
  dplyr::select(starts_with("Kids_First"), sample_id, id,
                pathology_free_text_diagnosis,
                pathology_diagnosis) %>%
  # group where there are duplicates
  group_by(Kids_First_Participant_ID, sample_id, id, pathology_diagnosis, pathology_free_text_diagnosis) %>%
...
lgat_snv <- read_tsv(file.path(subset_dir,"LGAT_snv_subset.tsv")) %>%
  left_join(lgat_manifest[,c("Kids_First_Biospecimen_ID", "id")]) %>%
...
methyl_subtyped <- read_tsv(file.path(subset_dir,"LGAT_methyl_subtypes.tsv")) %>%
  ...
  left_join(clinical_file[,c("Kids_First_Biospecimen_ID", "id", "pathology_diagnosis")]) %>%
  group_by(Kids_First_Participant_ID, molecular_subtype_methyl, id, pathology_diagnosis) %>% 
...
nf1_snv <- lgat_snv %>%
  ...
  dplyr::select("id","molecular_subtype") %>%
  unique()
...
```{r}
all_subtype_with_methyl <- methyl_subtyped[,c("Kids_First_Biospecimen_ID_Methyl", "molecular_subtype_methyl", "id")] %>%
  dplyr::full_join(all_subtype_clinical, by = "id") %>%
  dplyr::mutate(molecular_subtype = case_when(
  ...
  dplyr::select(Kids_First_Participant_ID, sample_id, Kids_First_Biospecimen_ID_DNA, Kids_First_Biospecimen_ID_RNA,
                Kids_First_Biospecimen_ID_Methyl, molecular_subtype, molecular_subtype_methyl, id) %>%
...
all_bs_ids <- clinical_file %>%
  ...
  left_join(all_subtype_with_methyl[,c("id", "molecular_subtype", "molecular_subtype_methyl")], by = "id") %>%
  select(Kids_First_Participant_ID,	Kids_First_Biospecimen_ID, sample_id, id, molecular_subtype, molecular_subtype_methyl) %>%
