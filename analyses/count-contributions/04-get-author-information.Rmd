...
split_name <- function(name) {
  ...
  if (ncol(parts) == 2) {
    return(tibble(Name = parts[1], `Middle Name(s)/Initial(s)` = "", `Last Name` = parts[2]))
  } else if (ncol(parts) == 3) {
    return(tibble(Name = parts[1], `Middle Name(s)/Initial(s)` = paste0(substr(parts[2], 1, 1), "."), `Last Name` = parts[3]))
  } else {
    return(tibble(Name = "", `Middle Name(s)/Initial(s)` = "", Last = ""))  # In case of unexpected format
  }
}
...
author_info <- author_info %>%
  dplyr::rename(Email = email,
                ORCiD = orcid) %>%
  dplyr::select(Email, affiliations, Name, `Middle Name(s)/Initial(s)`, `Last Name`, ORCiD, name)
```
