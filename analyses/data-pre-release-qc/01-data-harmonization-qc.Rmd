...
biospecimen_ids <- readr::read_tsv(file.path(data_dir,
                                        "gtex-gene-counts.liftover.tsv")) %>%
  colnames()
biospecimen_ids <- biospecimen_ids[grepl("GTEx", biospecimen_ids)]

missing_ids <- setdiff(hist_ids, biospecimen_ids)
missing_hist_ids <- setdiff(biospecimen_ids, hist_ids)
if (length(missing_ids) > 0) {
  output_file <- "histologies-samples-missing-in-gtex-gene-counts.liftover.tsv"
  missing_ids_df <- missing_biospecimen_table(hist_df, missing_ids, output_file)
  missing_ids_df
}
print(paste("RNA-Seq biospecimen in histolgies missing in gtex-gene-counts.liftover.tsv = ", length(missing_ids)))
...
biospecimen_ids <- readr::read_tsv(file.path(data_dir,
                                        "gtex-gene-expression-tpm.liftover.tsv")) %>%
  colnames()
biospecimen_ids <- biospecimen_ids[grepl("GTEx", biospecimen_ids)]
...
missing_hist_ids <- setdiff(biospecimen_ids, hist_ids)
if (length(missing_ids) > 0) {
  output_file <- "histologies-samples-missing-in-gtex-gene-expression-tpm.liftover.tsv"
  missing_ids_df <- missing_biospecimen_table(hist_df, missing_ids, output_file)
  missing_ids_df
}
print(paste("RNA-Seq biospecimen in histolgies missing in gtex-gene-expression-tpm.liftover.tsv = ", length(missing_ids)))
...
gtex_tpm <-
  tibble::tibble(GTEx_TPM =
    readr::read_rds(file.path(data_dir, "gtex-gene-expression-tpm.liftover.tsv")) %>%
...
gtex_counts <-
  tibble::tibble(GTEx_COUNTS =
    readr::read_rds(file.path(data_dir, "gtex-gene-counts.liftover.tsv")) %>%
      pull(gene_name) %>% unique()) %>%
...
biospecimen_ids <- readr::read_rds(file.path(data_dir,
                                        "cptac-protein-imputed-prot-expression.tsv")) %>%
  colnames()
biospecimen_ids <- biospecimen_ids[grepl("BS_", biospecimen_ids)]
missing_ids <- setdiff(hist_ids, biospecimen_ids)
missing_hist_ids <- setdiff(biospecimen_ids, hist_ids)
if (length(missing_ids) > 0) {
  output_file <- "histologies-samples-missing-in-cptac-protein-imputed-prot-expression.tsv"
  missing_ids_df <- missing_biospecimen_table(hist_df, missing_ids, output_file)
  missing_ids_df
}
print(paste("WCP biospecimen in histolgies missing in cptac-protein-imputed-prot-expression.tsv = ", length(missing_ids)))
if (length(missing_hist_ids) > 0) {
  output_file <- "cptac-protein-imputed-prot-expression-samples-missing-in-histologies.tsv"
  missing_hist_ids_df <- tibble::tibble(Kids_First_Biospecimen_ID = missing_hist_ids)
  missing_hist_ids_df %>% readr::write_tsv(file.path(results_dir, output_file))
  missing_hist_ids_df
}
print(paste("WCP biospecimen in cptac-protein-imputed-prot-expression.tsv missing in histolgies = ", length(missing_hist_ids)))
...
biospecimen_ids <- readr::read_rds(file.path(data_dir,
                                        "hope-protein-imputed-prot-expression.tsv")) %>%
  colnames()
biospecimen_ids <- biospecimen_ids[grepl("BS_", biospecimen_ids)]
missing_ids <- setdiff(hist_ids, biospecimen_ids)
missing_hist_ids <- setdiff(biospecimen_ids, hist_ids)
if (length(missing_ids) > 0) {
  output_file <- "histologies-samples-missing-in-hope-protein-imputed-prot-expression.tsv"
  missing_ids_df <- missing_biospecimen_table(hist_df, missing_ids, output_file)
  missing_ids_df
}
print(paste("WCP biospecimen in histolgies missing in hope-protein-imputed-prot-expression.tsv = ", length(missing_ids)))
if (length(missing_hist_ids) > 0) {
  output_file <- "hope-protein-imputed-prot-expression-samples-missing-in-histologies.tsv"
  missing_hist_ids_df <- tibble::tibble(Kids_First_Biospecimen_ID = missing_hist_ids)
  missing_hist_ids_df %>% readr::write_tsv(file.path(results_dir, output_file))
  missing_hist_ids_df
}
print(paste("WCP biospecimen in hope-protein-imputed-prot-expression.tsv missing in histolgies = ", length(missing_hist_ids)))
...
biospecimen_ids <- readr::read_rds(file.path(data_dir,
                                        "cptac-protein-imputed-phospho-expression.tsv")) %>%
  colnames()
biospecimen_ids <- biospecimen_ids[grepl("BS_", biospecimen_ids)]
missing_ids <- setdiff(hist_ids, biospecimen_ids)
missing_hist_ids <- setdiff(biospecimen_ids, hist_ids)
if (length(missing_ids) > 0) {
  output_file <- "histologies-samples-missing-in-cptac-protein-imputed-phospho-expression.tsv"
  missing_ids_df <- missing_biospecimen_table(hist_df, missing_ids, output_file)
  missing_ids_df
}
print(paste("Phospho biospecimen in histolgies missing in cptac-protein-imputed-phospho-expression.tsv = ", length(missing_ids)))
if (length(missing_hist_ids) > 0) {
  output_file <- "cptac-protein-imputed-phospho-expression-samples-missing-in-histologies.tsv"
  missing_hist_ids_df <- tibble::tibble(Kids_First_Biospecimen_ID = missing_hist_ids)
  missing_hist_ids_df %>% readr::write_tsv(file.path(results_dir, output_file))
  missing_hist_ids_df
}
print(paste("Phospho biospecimen in cptac-protein-imputed-prot-expression.tsv missing in histolgies = ", length(missing_hist_ids)))
...
missing_hist_ids <- setdiff(biospecimen_ids, hist_ids)
if (length(missing_ids) > 0) {
  output_file <- "histologies-samples-missing-in-hope-protein-imputed-phospho-expression.tsv"
  missing_ids_df <- missing_biospecimen_table(hist_df, missing_ids, output_file)
  missing_ids_df
}
print(paste("Phospho biospecimen in histolgies missing in hope-protein-imputed-phospho-expression.tsv = ", length(missing_ids)))
if (length(missing_hist_ids) > 0) {
  output_file <- "hope-protein-imputed-phospho-expression-samples-missing-in-histologies.tsv"
  missing_hist_ids_df <- tibble::tibble(Kids_First_Biospecimen_ID = missing_hist_ids)
  missing_hist_ids_df %>% readr::write_tsv(file.path(results_dir, output_file))
  missing_hist_ids_df
}
print(paste("Phospho biospecimen in hope-protein-imputed-prot-expression.tsv missing in histolgies = ", length(missing_hist_ids)))
