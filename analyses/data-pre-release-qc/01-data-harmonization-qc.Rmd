...
hist_ids <- hist_df %>%
  filter(experimental_strategy == "Whole Cell Proteomics" & cohort == "PBTA" & sub_cohort == "CPTAC") %>%
  dplyr::pull(Kids_First_Biospecimen_ID)
...
hist_ids <- hist_df %>%
  filter(experimental_strategy == "Whole Cell Proteomics" & cohort == "PBTA" & sub_cohort == "HOPE") %>%
  dplyr::pull(Kids_First_Biospecimen_ID)
...
hist_ids <- hist_df %>%
  filter(experimental_strategy == "Phospho-Proteomics" & cohort == "PBTA" & sub_cohort == "CPTAC") %>%
  dplyr::pull(Kids_First_Biospecimen_ID)
...
hist_ids <- hist_df %>%
  filter(experimental_strategy == "Phospho-Proteomics" & cohort == "PBTA" & sub_cohort == "HOPE") %>%
  dplyr::pull(Kids_First_Biospecimen_ID)
...
```{r cnv tumor only}
# check that all  biospecimen IDs are in the file
biospecimen_ids <- readr::read_tsv(file.path(data_dir, "cnv-controlfreec-tumor-only.tsv.gz")) %>%
  dplyr::pull(Kids_First_Biospecimen_ID) %>%
  unique()
hist_ids <- hist_df %>%
  dplyr::filter(experimental_strategy %in% c("WGS", "WXS", "Targeted Sequencing"),
...
missing_hist_ids <- setdiff(biospecimen_ids, hist_ids)
if (length(missing_ids) > 0) {
  output_file <- "histologies-samples-missing-in-cnv-controlfreec-tumor-only.tsv"
  missing_ids_df <- missing_biospecimen_table(hist_df, missing_ids, output_file)
  missing_ids_df
}
print(paste("DNA biospecimen in histolgies missing in cnv-controlfreec-tumor-only.tsv.gz = ", length(missing_ids)))
if (length(missing_hist_ids) > 0) {
...
print(paste("DNA biospecimen in cnv-controlfreec-tumor-only.tsv.gz missing in histolgies = ", length(missing_hist_ids)))
```
