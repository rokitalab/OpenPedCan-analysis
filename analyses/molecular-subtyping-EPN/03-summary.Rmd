...
EPN_final <- read_tsv("results/EPN_all_data.tsv") %>%
  mutate(molecular_subtype = case_when(is.na(Kids_First_Biospecimen_ID_DNA) & is.na(Kids_First_Biospecimen_ID_RNA) &
...
EPN_final <- EPN_final %>%
  mutate(molecular_subtype = ifelse(molecular_subtype == "EPN, To be classified" & tmp == TRUE, "EPN, ST ZFTA", molecular_subtype)) %>%
  dplyr::select(-c(tmp))
...
EPN_final$tmp[is.na(EPN_final$tmp)] <- FALSE
EPN_final <- EPN_final %>%
  mutate(molecular_subtype = ifelse(molecular_subtype == "EPN, To be classified" & tmp == TRUE, "EPN, ST YAP1", molecular_subtype)) %>%
  dplyr::select(-c(tmp))
...
1. Here we will assign the PT_EPN_A and PT_EPN_B molecular_subtypes to the sample that have not been molecular_subtypeed before. 
...
EPN_final <- EPN_final %>%
  mutate(molecular_subtype = case_when(
    molecular_subtype == "EPN, To be classified" & `1q_gain` > 0 & TKTL1_expr_zscore > 3 ~  "EPN, PF A",
    molecular_subtype == "EPN, To be classified" & EZHIP_expr_zscore > 3 ~ "EPN, PF A",
    molecular_subtype == "EPN, To be classified"  & CNS_region == "Posterior fossa" & 
    ...
    molecular_subtype == "EPN, To be classified" & (`6q_loss` > 0 | `6p_loss` > 0) & 
      (GPBP1_expr_zscore > 3 | IFT46_expr_zscore > 3) ~ "EPN, PF B",
    molecular_subtype == "EPN, To be classified" & !is.na(molecular_subtype_methyl) ~ molecular_subtype_methyl,
...
EPN_final_subtyped <- EPN_final %>% 
  mutate(molecular_subtype = ifelse(is.na(molecular_subtype), "N/A", molecular_subtype)) %>%
...
EPN_final_subtyped %>%
  filter(`ZFTA--RELA` == 1 | `ZFTA--MAML2` == 1, `PTEN--TAS2R1` == 1, `YAP1--FAM118B` == 1, `YAP1--MAML2` == 1) %>%
  select(id, Kids_First_Biospecimen_ID_RNA, Kids_First_Biospecimen_ID_RNA_panel, `ZFTA--RELA`, `ZFTA--MAML2`, `PTEN--TAS2R1`, `YAP1--FAM118B`, `YAP1--MAML2`,
        molecular_subtype, molecular_subtype_methyl)
