...
mb_var_probes <- names(sort(mb_methyl_var, decreasing = TRUE)[1:10000])
```
...
ggsave(file.path(plots_dir, "umap_mb.tiff"),
...
mb_g34_var_probes <- names(sort(mb_g34_methyl_var, decreasing = TRUE)[1:10000])
```
...
ggsave(file.path(plots_dir, "umap_mb_group34.tiff"),
       width = 5.5, height = 3.5)
```
...
mb_shh_var_probes <- names(sort(mb_shh_methyl_var, decreasing = TRUE)[1:10000])
```
...
ggsave(file.path(plots_dir, "umap_mb_shh_methylation_subtype.tiff"),
       width = 5.5, height = 3.5)
```
...
ggsave(file.path(plots_dir, "umap_mb_shh_cn_status.tiff"),
       width = 8, height = 5.5)
```
...
mb_wnt_var_probes <- names(sort(mb_wnt_methyl_var, decreasing = TRUE)[1:10000])
```
...
ggsave(file.path(plots_dir, "umap_mb_wnt.tiff"),
       width = 5.5, height = 3.5)
```
