...
pb_meta <- histo %>% 
  ...
  mutate(molecular_subtype_methyl = case_when(
    dkfz_v12_methylation_subclass == "PB_FOXR2" ~ "PB, FOXR2", 
    dkfz_v12_methylation_subclass == "PIN_RB" ~ "PIN, RB1",
...
### pineoblastoma, MYC/FOXR2-activated (PB, MYC)
...
pb_methy_with_subtype <- pb_meta %>% 
  ...
  mutate(molecular_subtype = case_when(molecular_subtype_methyl == "PB, FOXR2" ~ "PB, FOXR2", 
                                       molecular_subtype_methyl == "PIN, RB1" ~ "PB, RB1",
...
pb_meta_with_mol <- pb_meta %>%
  ...
  left_join(pb_methyl_df, by = "sample_id") %>% 
  select(Kids_First_Biospecimen_ID, Kids_First_Participant_ID, sample_id, molecular_subtype_methyl, molecular_subtype) %>% 
  ...
  write_tsv(file.path(result_dir, "pb-molelar-subtype.tsv"))
