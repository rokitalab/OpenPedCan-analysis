...
dgd_df <- read_tsv(dgd_maf_file, comment = '#') %>%
  ...
  mutate(n_ref_count = as.numeric(n_ref_count),
...
dnameta <-
  ...
  filter(experimental_strategy %in% c("WGS","WXS", "Targeted Sequencing"),
         is.na(RNA_library)) %>%
  #group_by(Kids_First_Participant_ID, sample_id, age_at_diagnosis_days) %>%
   #  summarise(Kids_First_Biospecimen_ID = str_c(Kids_First_Biospecimen_ID, collapse=",")) %>%
...
meta_subtype <-
  full_join(allmeta,
            ...
            by=c("Kids_First_Participant_ID","sample_id")) %>%
  group_by(Kids_First_Participant_ID, sample_id, age_at_diagnosis_days) %>%
     summarise(Kids_First_Biospecimen_ID_DNA = str_c(unique(Kids_First_Biospecimen_ID_DNA), collapse=","),
               Kids_First_Biospecimen_ID_RNA = str_c(unique(Kids_First_Biospecimen_ID_RNA), collapse=","),
               # Add this to combine and fix subtypes if subtyped as ADAM + To be classified
               molecular_subtype = str_c(unique(molecular_subtype), collapse=";")) %>%
  dplyr::mutate(molecular_subtype = case_when(molecular_subtype == "CRANIO, To be classified;CRANIO, ADAM" ~ "CRANIO, ADAM",
                                              molecular_subtype == "CRANIO, ADAM;CRANIO, To be classified" ~ "CRANIO, ADAM",
