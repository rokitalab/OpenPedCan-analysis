...
results_path       <- file.path(analysis_path, "mutational-signatures", "results", output_Folder)
plot_path       <- file.path(analysis_path, "mutational-signatures", "plots", output_Folder, "cns")
if(!dir.exists(plot_path)){
  dir.create(plot_path)
}

# Input files
fitted_file    <- file.path(results_path, "deconstructSigs_cns_exposures_merged.tsv")
meta_file      <- file.path(proj_root_path, "data", "histologies.tsv")
tmb_all_file    <- file.path(proj_root_path, "data", "snv-mutation-tmb-all.tsv")
tmb_coding_file <- file.path(proj_root_path, "data", "snv-mutation-tmb-coding.tsv")
...
tmb_sigs_plot_file <- file.path(plot_path, "tmb_signature_dotplot.pdf")
sig_matrix_file <- file.path(results_path, "hypermutator_sig_matrix.tsv")
subtype_sig_file <- file.path(results_path, 'sig_matrix_by_molecular_subtype.tsv')
```
...
# Read in exposures, setting the column types so that signature column is read as character to avoid parsing failures
...
tmb_sigs_df <- exposures %>%
  dplyr::select(signature, exposure, Kids_First_Biospecimen_ID, cancer_group, broad_histology) %>%
  dplyr::distinct() %>%
...
tmb_sigs_plot <- tmb_sigs_df %>%
  ...
  ggplot2::theme(legend.title = element_text(size = 4), 
               legend.text = element_text(size = 4)) + 
  ggplot2::guides(fill=guide_legend(ncol=2))
  ggplot2::ggsave(tmb_sigs_plot_file, tmb_sigs_plot, width = 7.5, height = 4, 
                 useDingbats = FALSE)
```
...
if (!(params$is_ci == 1)) {
  ...
  hypermutator_df <- tmb_sigs_df %>%
    ...
    # we don't need cancer_group, the info is in molecular_subtype
    dplyr::select(-cancer_group) %>%
    # arrange in order of burden
    dplyr::arrange(-coding_tmb) %>%
    write_tsv(subtype_sig_file)
  
  hypermutator_df
} 
```

### Make mutational signatures matrix for patients with hypermutant tumors
```{r}
# This chunk should NOT run in CI. 
if (!(params$is_ci == 1)) {
  ...
  sigs <- exposures %>%
    ...
    tidyr::spread(signature, exposure) %>%
    select(-broad_histology, -tumor_descriptor) %>%
