...
calc_mut_per_sig <- function(which_sig_list,
  ...
  sig_num_df <- do.call(
    ...
    # Add genome/exome size and calculate the mutation per this column
    left_join(region_sizes, by = 'Tumor_Sample_Barcode') %>%
    ...
    dplyr::mutate(
      ...
      #                             #        "WXS" = wxs_size
      # ),
      mut_per_mb = num_mutations / (region_size / 10^6)
    ) %>% 
