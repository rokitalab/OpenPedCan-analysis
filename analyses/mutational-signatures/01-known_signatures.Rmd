...
Create list of include/exclude signatures using SBSv3 mapping file 
...
```{r Set up lists of excluded and included signatures for COSMIC V3}
 ...
 exclude_sigs <- c(artifact_sigs, environ_sigs, unknown, "SBS39")
 include_sigs <- setdiff(rownames(signatures.cosmic.v3.3), exclude_sigs)
```
...
sample_sigs_cosmic_v33 <- lapply(tumor_sample_ids, function(sample_id) {
  ...
  whichSignatures(
    tumor.ref = sigs_input,
    signatures.ref = signatures.cosmic.v3.3[include_sigs,],
    sample.id = sample_id,
    contexts.needed = TRUE
    )
})
