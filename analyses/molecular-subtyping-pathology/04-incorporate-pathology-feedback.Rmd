...
compiled_df <- read_tsv(compiled_results_file, guess_max = 100000)
fusions_df <- read_tsv(fusions_file, guess_max = 100000)
```
...
compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      sample_id == "7316-1106" ~ "Pleomorphic xanthoastrocytoma",
      TRUE ~ integrated_diagnosis
    ),
    short_histology = case_when(
      sample_id == "7316-1106" ~ "HGAT",
      TRUE ~ short_histology
    ),
    broad_histology = case_when(
      sample_id == "7316-1106" ~ "Diffuse astrocytic and oligodendroglial tumor",
      TRUE ~ broad_histology
    ),
    molecular_subtype = case_when(
      sample_id == "7316-1106" ~ "HGG, PXA",
...
compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      sample_id == "7316-1102" ~ "Diffuse midline glioma, H3 K28-altered",
      TRUE ~ integrated_diagnosis
    ),
    short_histology = case_when(
...
compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      sample_id == "7316-158" ~ "Diffuse hemispheric glioma, H3 G35-mutant",
      TRUE ~ integrated_diagnosis
    ),
    short_histology = case_when(
...
compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      sample_id == "7316-1105" ~ "Diffuse hemispheric glioma, H3 G35-mutant",
      TRUE ~ integrated_diagnosis
    ),
    short_histology = case_when(
    ...
    molecular_subtype = case_when(
      sample_id == "7316-1105" ~ "DHG, H3 G35",
      TRUE ~ molecular_subtype
    ),
...
compiled_df <- compiled_df %>%
    mutate(molecular_subtype = case_when(
      ...
      TRUE ~ molecular_subtype),
   harmonized_diagnosis = case_when(molecular_subtype == "CHDM, poorly differentiated" ~ "Chordoma, poorly differentiated",
                                    molecular_subtype == "CHDM, conventional" ~ "Chordoma, conventional",
                                    molecular_subtype == "CHDM, To be classified" ~ NA_character_,
...
### PT `PT_7HGY5W67`
This patient subtyped as HGNET-MN1 via methylation, but GNT otherwise. Needs pathology review, keeping path dx for now.
```{r}
compiled_df %>%
  filter(Kids_First_Participant_ID == "PT_7HGY5W67")

compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      sample_id == "7316-6044" ~ "Glial-neuronal tumor, wildtype",
      TRUE ~ integrated_diagnosis
    ),
    short_histology = case_when(
      sample_id == "7316-6044" ~ "GNT",
      TRUE ~ short_histology
    ),
    broad_histology = case_when(
      sample_id == "7316-6044" ~ "Neuronal and mixed neuronal-glial tumor",
      TRUE ~ broad_histology
    ),
    molecular_subtype = case_when(
      sample_id == "7316-6044" ~ "GNT, wildtype",
      TRUE ~ molecular_subtype
    ),
    Notes = case_when(
      sample_id == "7316-6044" ~ "Updated via OpenPedCan subtyping",
      TRUE ~ Notes
    )
  )
```
...
### PT `PT_VZEREHVW`
This patient is being subtyped as both `CNS HGNET-MN1` and `HGG, To be classified`. Other biospecimen for the sample_id are all `Diffuse astrocytic and oligodendroglial tumor`. Exclduing `CNS HGNET-MN1`
```{r}
compiled_df %>%
  filter(Kids_First_Participant_ID == "PT_VZEREHVW")

compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      sample_id == "7316-6047" ~ NA_character_,
      TRUE ~ integrated_diagnosis
    ),
    short_histology = case_when(
      sample_id == "7316-6047" ~ "HGAT",
      TRUE ~ short_histology
    ),
    broad_histology = case_when(
      sample_id == "7316-6047" ~ "Diffuse astrocytic and oligodendroglial tumor",
      TRUE ~ broad_histology
    ),
    molecular_subtype = case_when(
      sample_id == "7316-6047" ~ "HGG, To be classified",
      TRUE ~ molecular_subtype
    ),
    Notes = case_when(
      sample_id == "7316-6047" ~ "Updated by manual curation of pathology_free_text_diagnosis",
      TRUE ~ Notes
    )
  ) %>% 
  distinct()
...
### PT `PT_01MZ62KG`
This patient is being subtyped as CNS embryonal NOS and has pathology free text diagnosis as pineoblastoma, but subtyped as NOS there as well. We will update to ETMR, C19MC-altered as previous.
```{r}
compiled_df %>%
  filter(Kids_First_Participant_ID == "PT_VZEREHVW")
...
compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      sample_id == "7316-447" ~ "Embryonal tumor with multilayer rosettes, C19MC-altered",
      TRUE ~ integrated_diagnosis
    ),
      short_histology = case_when(
      sample_id == "7316-447" ~ "ETMR",
      TRUE ~ short_histology
    ),
    broad_histology = case_when(
      sample_id == "7316-447" ~ "Embryonal tumor",
      TRUE ~ broad_histology
    ),
    molecular_subtype = case_when(
      sample_id == "7316-447" ~ "ETMR, C19MC-altered",
      TRUE ~ molecular_subtype
    ),
    Notes = case_when(
      sample_id == "7316-447" ~ "Updated by pathology review",
      TRUE ~ Notes
    )
  ) %>% 
  distinct()
...
### PT `PT_FMS4AXKD`
This patient 7316-9049 has K28-altered due to EGFR mutation, via free text, so we will add here
```{r}
compiled_df %>%
  filter(Kids_First_Participant_ID == "PT_FMS4AXKD")

compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      match_id == "PT_FMS4AXKD_7316-9049_Solid Tissue_Initial CNS Tumor" ~ "Diffuse midline glioma, H3 K28-altered (EGFR mutant)",
      TRUE ~ integrated_diagnosis
    ),
    molecular_subtype = case_when(
      match_id == "PT_FMS4AXKD_7316-9049_Solid Tissue_Initial CNS Tumor" ~ "DMG, H3 K28",
      TRUE ~ molecular_subtype
    ),
    Notes = case_when(
      match_id == "PT_FMS4AXKD_7316-9049_Solid Tissue_Initial CNS Tumor" ~ "Updated by pathology free text review",
      TRUE ~ Notes
    )
  ) %>% 
  distinct()
...
### PT `PT_E9J8FNW5`
This patient BS_Q2R56X78 has EGFR p.Gly598Val,TERT promoter, and H3C3 K28M (6:26045493) ~ 50% - not sure why this is not being called, need to investigate.
```{r}
compiled_df %>%
  filter(Kids_First_Participant_ID == "PT_E9J8FNW5")

compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      match_id == "PT_E9J8FNW5_C4116318-Tumor_Solid Tissue_Recurrence" ~ "Diffuse midline glioma, H3 K28-altered",
      TRUE ~ integrated_diagnosis
    ),
    molecular_subtype = case_when(
      match_id == "PT_E9J8FNW5_C4116318-Tumor_Solid Tissue_Recurrence" ~ "DMG, H3 K28",
      TRUE ~ molecular_subtype
    ),
    Notes = case_when(
      match_id == "PT_E9J8FNW5_C4116318-Tumor_Solid Tissue_Recurrence" ~ "Updated by pathology free text review",
      TRUE ~ Notes
    )
  ) %>% 
  distinct()
...
### PT `PT_FS1GKF9Y`
This patient BS_QV60J6XZ has A LOT of frameshifts with H3-3A 3' FLANK chr1:g.226072141del, TMB 143 (ultra-hypermutant), two EGFR dels p.Q976Sfs*26 and p.L405Cfs*2 - 2/19 reads - why wasn't this called? need to investigate
```{r}
compiled_df %>%
  filter(Kids_First_Participant_ID == "PT_FS1GKF9Y")

compiled_df <- compiled_df %>%
  mutate(
    integrated_diagnosis = case_when(
      match_id == "PT_FS1GKF9Y_C4115949-Tumor_Solid Tissue_Recurrence" ~ "Diffuse midline glioma, H3 K28-altered",
      TRUE ~ integrated_diagnosis
    ),
    molecular_subtype = case_when(
      match_id == "PT_FS1GKF9Y_C4115949-Tumor_Solid Tissue_Recurrence" ~ "DMG, H3 K28, TP53",
      TRUE ~ molecular_subtype
    ),
    Notes = case_when(
      match_id == "PT_FS1GKF9Y_C4115949-Tumor_Solid Tissue_Recurrence" ~ "Updated by pathology free text review",
      TRUE ~ Notes
    )
  ) %>% 
  distinct()
...
### exclude "NA" in methyl subtype column where sample has subtype

```{r}
compiled_df <- compiled_df %>%
  select(-molecular_subtype_methyl) %>%
  distinct() %>%
  left_join(compiled_df %>%
