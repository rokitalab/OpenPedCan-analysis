...
histologies_df <- read_tsv(histologies_file, 
  ...
  dplyr::mutate(match_id = paste(sample_id, composition, sep = "_"))
...
emb_rna <- relevant_clinical_data %>%
  filter(experimental_strategy %in% c("RNA-Seq", "Targeted Sequencing"),
         !is.na(RNA_library)) %>%
  select(Kids_First_Biospecimen_ID,
         Kids_First_Participant_ID, 
         sample_id, composition, match_id,
...
all_data_df <- full_join(emb_dna, emb_rna)
...
all_data_df %>%
  select(Kids_First_Participant_ID, 
         ...
         MN1_fusions,
         CIC_fusions) %>%
  arrange(Kids_First_Participant_ID, sample_id, composition, match_id) %>%
...
subtypes_df <-  readr::read_tsv(methyl_file) %>% 
  dplyr::full_join(subtypes_df, by = c("Kids_First_Participant_ID", "sample_id")) %>% 
