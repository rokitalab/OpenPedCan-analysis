...
if ( params$base_run ==0 ){
  clinical<-read_tsv(file.path(data_dir,"histologies.tsv"), guess_max = 100000) %>%
  filter(!is.na(pathology_diagnosis),
         !cohort %in% c("TCGA", "GTEx")) %>%
    mutate(sample_id = case_when(cohort %in% c("TARGET", "GMKF") ~ paste(Kids_First_Participant_ID, sample_id, tumor_descriptor, sep = "_"),
                                 TRUE ~ sample_id))
} else{
  clinical<-read_tsv(file.path(data_dir,"histologies-base.tsv"), guess_max = 100000) %>%
  filter(!is.na(pathology_diagnosis),
         !cohort %in% c("TCGA", "GTEx")) %>%
    mutate(sample_id = case_when(cohort %in% c("TARGET", "GMKF") ~ paste(Kids_First_Participant_ID, sample_id, tumor_descriptor, sep = "_"),
                                 TRUE ~ sample_id))
...
putative_onco_fusion <- read_tsv(file.path(data_dir, "fusion-putative-oncogenic.tsv"))
manta_sv <- vroom(file.path(data_dir, "sv-manta.tsv.gz")) %>%
  filter(`Kids.First.Biospecimen.ID.Tumor` %in% clinical$Kids_First_Biospecimen_ID)
...
Since we want to capture TP53 loss phenotypes we will only keep breakpoints that are either DEL: deletion, INV: inversion and BND: translocations which are breakpoint notations with 2 breakpoints in different chromosomes
...
putative_onco_tp53_fusion <- putative_onco_fusion %>%
  ...
  dplyr::select("Sample",
                ...
                "RightBreakpoint",
                "LeftBreakpoint") %>%
  reshape2::melt(c("Sample","sample_id", "tumor_descriptor", "composition", "FusionName"))
```
...
lowexpr_tp53_with_sv <- rna_expr["TP53", ] %>%
  ...
  # remove GTEX
  filter(!grepl("GTEX", Kids_First_Biospecimen_ID)) %>%
