...
sv_overlap <- read_tsv(
  file.path(
    results_dir,
    "sv_overlap_tp53.tsv")) %>%
  left_join(histology, by=c("Kids_First_Biospecimen_ID", "sample_id", "tumor_descriptor", "composition"))
...
tp53_alterations <- histology %>%
  ...
  left_join(cnv_domain_overlap,by=c("Kids_First_Biospecimen_ID"="biospecimen_id",
                                    "sample_id", "tumor_descriptor", "Kids_First_Participant_ID", 
                                    "composition")) %>%
  # join SV 
  left_join(sv_overlap,by=c("Kids_First_Biospecimen_ID", "sample_id", "tumor_descriptor", "composition")) %>%
  dplyr::rename(Kids_First_Biospecimen_ID_DNA = Kids_First_Biospecimen_ID) %>%
  # join fusion
  left_join(fusion_overlap,by=c("sample_id", "tumor_descriptor", "composition")) 
